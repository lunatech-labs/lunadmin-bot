...
class TaskScheduler @Inject()(s: Starter, conf: Configuration)(implicit actorSystem: ActorSystem, ec: ExecutionContext) {
  ...
  // initialization in case of restart
    ...
    s.userDataStore.findUserById(task.employeeId).foreach { optUser =>
      optUser.foreach { user =>
        if (Seq("Unique - Common", "Unique - Important").contains(task.status)) {
          scheduleASlackBotMessageOnce(task._id, user.mail, task.startDate, task.endDate, task.status, task.description)
          for (alert <- task.alert) {
            scheduleASlackBotAlertMessageOnce(task._id, user.mail, task.startDate, task.endDate, task.status, task.description, alert._1)
          }
        } else {
  ...
  private def scheduleASlackBotMessageOnce(taskId: String, mail: String, startDate: ZonedDateTime,endDate : ZonedDateTime, taskStatus: String, taskDescription: String): Unit = {
    val timeLeftUntilStartDate: Long = startDate.toInstant.toEpochMilli - ZonedDateTime.now().toInstant.toEpochMilli

    taskStatus match {
      case "Unique - Common" =>
        scheduleAMessageOnceAndAddToCancellableList(taskId, taskStatus, timeLeftUntilStartDate, mail, taskDescription, startDate, endDate, missionTitle, communColor)
      case "Unique - Important" =>
        scheduleAMessageOnceAndAddToCancellableList(taskId, taskStatus, timeLeftUntilStartDate, mail, taskDescription, startDate, endDate, missionTitle, importantColor)
    }
  }

  private def scheduleASlackBotAlertMessageOnce(taskId: String, mail: String, startDate: ZonedDateTime, endDate : ZonedDateTime, taskStatus: String, taskDescription: String, alertDuration: Long): Unit = {
    ...
    scheduleAMessageOnceAndAddToCancellableList(taskId, taskStatus, timeLeftUntilStartDate, mail, taskDescription, startDate, endDate, reminderTitle, reminderColor)
  }
  ...
  private def makeRecurrentMessage(taskDescription: String, slackUser: User, startDate: ZonedDateTime, taskStatus: String) = {
    val userDate = startDate.withZoneSameInstant(ZoneId.of(slackUser.tz.getOrElse("UTC")))
    taskStatus match {
      case "Daily" => s"$taskDescription - $taskStatus - Every day at ${userDate.getHour}h${userDate.getMinute} "
      case "Weekly" => s"$taskDescription - $taskStatus - Every ${userDate.getDayOfWeek.getDisplayName(TextStyle.FULL, Locale.ENGLISH)} at ${userDate.getHour}h${userDate.getMinute}"
      case "Monthly" => s"$taskDescription - $taskStatus - The ${userDate.getDayOfMonth} day of every month at ${userDate.getHour}h${userDate.getMinute} "
      case "Yearly" => s"$taskDescription - $taskStatus - The ${userDate.getDayOfMonth} of ${userDate.getMonth.getDisplayName(TextStyle.FULL, Locale.ENGLISH)} at ${userDate.getHour}h${userDate.getMinute} every year"
    }
  }
  ...
  private def scheduleAMessageOnceAndAddToCancellableList(taskId: String, taskStatus: String, timeLeftUntilStartDate: Long, mail: String, taskDescription: String, startDate: ZonedDateTime,endDate : ZonedDateTime, title: String, color: String): Unit = {
  ...
  private def scheduleEndOfMessage(taskId: String, endDate: ZonedDateTime): Unit = {
    ...
    val scheduledEnd = actorSystem.scheduler.scheduleOnce(delay millis) {
      deleteTask(taskId)
      s.taskDataStore.setTaskToInactive(taskId)
    }
  ...
  def initializeTaskSchedules() : Unit = {
    val lists = for{
      listOfSingle <- s.taskDataStore.findAllActiveSingleTask()
      listOfGrouped <- s.taskDataStore.findAllActiveGroupedTask()
    } yield (listOfSingle,listOfGrouped)

    lists.map{l =>
      for(sT <- l._1){
        if(sT.endDate.toInstant.toEpochMilli < ZonedDateTime.now().toInstant.toEpochMilli){
          s.taskDataStore.setTaskToInactive(sT._id)
        }else{
          setSlackBotMessageForSingleTask(sT)
        }
      }
      for(gT <- l._2){
        if(gT.endDate.toInstant.toEpochMilli < ZonedDateTime.now().toInstant.toEpochMilli){
          s.taskDataStore.setTaskToInactive(gT._id)
        }else{
          setSlackBotMessageForGroupedTask(gT)
        }
      }
    }
  }
